...
const createTask = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: "สร้าง Task สำเร็จ", task })
    } catch (error) {
        res.status(500).json({ message: "Server Error", error: error.message })
    }
}
...
const updateTask = async (req, res) => {
    try {
        ...
        res.json({ message: "อัพเดต Task เรียบร้อย", updateTask })
    } catch (error) {
        res.status(500).json({ message: "Server Error", error: error.message })
    }
}
...
const updateTaskStatus = async (req, res) => {
    try {
        ...
        await task.save()
        res.json({ message: "อัพเดต Status Task เรียบร้อย", task })
    } catch (error) {
        res.status(500).json({ message: "Server Error", error: error.message })
    }
}
...
const updateTaskChecklist = async (req, res) => {
    try {
        ...
        res.json({ message: "อัพเดต Task Checklist เรียบร้อย", task: updateTask })
    } catch (error) {
        res.status(500).json({ message: "Server Error", error: error.message })
    }
}
const getDashboardData = async (req, res) => {
    try {
        ...
        const taskStatuses = ["Pending", "In Process", "Completed"]
